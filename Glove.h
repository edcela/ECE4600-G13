...
public:
    ...
    uint8_t getOrientation(){}    //Returns the orientation of the gesture. If multiple orientations are allowed, only the first orientation is returned.
    ...
    uint8_t getFingerStates() {}    //Returns flexion states. As in the gestureID of this gesture with orientation omitted.
    ...
    void setDroneID(int droneNum){} //Assigns a DroneID for this gesture. Only one ID is allowed.
    int getDroneID(){}
...
private:
    ...
    int droneID;                        //Corresponding drone ID for this gesture
...
public:
    GestureBank() {};
    ~GestureBank() {};
    void addGesture(gesture newGesture);
    std::vector<gesture>::iterator findGesture(uint8_t gestureToFind); //Finds a known gesture using ID value
private:
